...
void* LU_threadFunc_8threads(void* param) { //平凡算法，动态分配8线程
...
void LU_pthread_dynamic_8threads() {    //平凡算法，动态分配8线程
    for (int k = 0; k < N; k++) {
        for (int j = k + 1; j < N; j++) {
            A[k][j] = A[k][j] / A[k][k];
        }
        A[k][k] = 1.0;
        //创建工作线程，进行消去工作
        int worker_cnt = NUM_THREADS;//工作线程数量
        ...
        for (int t_id = 0; t_id < worker_cnt; t_id++) {
            pthread_create(&handles[t_id], NULL, LU_threadFunc_8threads, &param[t_id]);
        }
...
int main() {
    ...
    test(LU_pthread_dynamic_8threads);
